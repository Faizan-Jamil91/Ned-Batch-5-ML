...
def main():
    ...
    if not st.session_state['logged_in']:
        ...
        generate_mcqs_section()
        if 'mcqs' in st.session_state:
            generate_result_section()  # Display the "Generate result" button only if MCQs have been generated
        logout_button()
        ...
        if st.button("Generate MCQs"):
            try:
                ...
                with st.spinner('Generating MCQs...'):
                    response = requests.post(f"http://127.0.0.1:8000/generate_mcqs/?topic_input={topic_input}", headers=headers)
                    response.raise_for_status()
    ...
    if st.button("Generate result"):
        try:
            ...
            with st.spinner('Generating Result...'):
                ...
                response = requests.post(f"http://127.0.0.1:8000/generate_result/?result={result}&collected_answers={collected_answers}", headers=headers)
                response.raise_for_status()
